...
def retrieve_tasks(req):
    ...
    try:
        task = Tasks.objects.filter(project=data.get('project')).values('task_id', 'feature', 'status', 'assigned', 'sprint', 'priority','starting_date', 'deadline')
...
def update_project_description(req):
    ...
    try:
        ...
        project.save()
        return Response({"message": "Project description updated successfully!"}, status=200)
